...
if (nlhs > 2) {
	mexErrMsgTxt("Error: Number of output arguments must be less than 2.");
}
if (nrhs != 3) {
...
double *intersectedVoxelTimes; //storage for the indexes of the intersected voxels - dynamically allocated
double t;
int addedVoxelCount;
...
plhs[1]= mxCreateNumericMatrix(storageSize,1,mxDOUBLE_CLASS,mxREAL); // initializes to zeros
intersectedVoxelTimes = (double*)mxGetData(plhs[1]);
...
while ((X!=Xend)||(Y!=Yend)||(Z!=Zend))
{
    ...
    if (addedVoxelCount>storageSize)
    {
        ...
        intersectedVoxelData = (int*)mxRealloc(intersectedVoxelData, sizeof(int)*storageSize);
				intersectedVoxelTimes = (double*)mxRealloc(intersectedVoxelTimes, sizeof(int)*storageSize);
    }
    intersectedVoxelData[addedVoxelCount-1] = sub2ind(X, Y, Z, gridSize);
		intersectedVoxelTimes[addedVoxelCount-1] = t;
}
...
addedVoxelCount++;
intersectedVoxelTimes[addedVoxelCount-1] = tMax;
intersectedVoxelTimes = (double*)mxRealloc(intersectedVoxelTimes, sizeof(int)*addedVoxelCount);
mxSetM(plhs[1], addedVoxelCount); //number of rows
mxSetN(plhs[1], 1); //number of columns
mxSetData(plhs[1], intersectedVoxelTimes); // update pointer to the matrix data
}

int sub2ind(const int X, const int Y, const int Z, const double * gridSize)
{
	// return (Y + (X - 1)*gridSize[0] + (Z - 1)*gridSize[1]*gridSize[0]);
	return (1 - Y + X * gridSize[1] + (Z - 1) * gridSize[1] * gridSize[0]);
}
